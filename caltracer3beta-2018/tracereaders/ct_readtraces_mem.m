function [traces, halo_traces, param] = ct_readtraces_mem(experiment, ...
						  maskidx,preferences)
% Program used by caltracer3beta.
...
assignin('base','contours',contours)
...
if (experiment.haloMode == 1)
	...
	waitbar(c/contourslen);
        ...
        outsideim=fx<1|fx>nx;%find x-axis components that are outside image
        fx(outsideim)=[];%delete them 
        fy(outsideim)=[];%and the y coords for the corresponding points
        outsideim=fy<1|fy>ny;    %repeat with y's that are outside
        fx(outsideim)=[];
        fy(outsideim)=[];
