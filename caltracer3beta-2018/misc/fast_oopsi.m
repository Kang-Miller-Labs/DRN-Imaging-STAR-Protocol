...
%%
preprocess_opt = 0;%input('\ndoes the data need to be preprocessed? [0 - no 1 - yes]: ');
...
if ~isfield(P,'sig'),   P.sig   = mean(mad(F',1)/1.4785);       end
if ~isfield(P,'gam'),   P.gam   = (1-V.dt/.8)*ones(V.Ncells,1);  end
